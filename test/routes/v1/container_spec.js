...
describe('API Container ', () => {
  ...
  it('should return error because there is no container enough big to save the product', (done) => {
    const clientId = 1;
    const requestId = uniqid();
    chai
      .request(URL)
  ...
  it('should return one container using the skip and limit and then return two containers using skip and limit with other parameters', (done) => {
    chai
      ...
      .end(function (err, res) {
        expect(err).to.equal(null);
        const firstContainerId = res.body.data.id;
        expect(res).to.have.status(201);
        expect(res.body).to.have.status('OK');
        chai
          .request(URL)
          ...
          .end(function (err, res) {
            expect(err).to.equal(null);
            const secondContainerId = res.body.data.id;
            expect(res).to.have.status(201);
            expect(res.body).to.have.status('OK');
            chai
              .request(URL)
